...
module('Integration | Component | message', function (hooks) {
  ...
  test('it renders a message model', async function (assert) {
    ...
    assert.strictEqual(
      this.element.querySelector('[data-test-sender]').textContent.trim(),
      'testing bot',
      'sender is correct'
    );
  ...
  test('the message content can be edited', async function(assert) {
    ...
    let editButton = this.element.querySelector('[data-test-edit-button]');
    await click(editButton);

    let contentInput = this.element.querySelector('[data-test-content-input]');
    assert.dom(contentInput).exists();

    assert.strictEqual(contentInput.value, 'something that will not be here by coincidence');
